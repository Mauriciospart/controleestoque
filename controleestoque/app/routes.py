...
def products():
    ...
    if search:
        products = Product.query.filter(Product.is_active==True, Product.name.like(f'%{search}%')).paginate(page=page, per_page=10)
    else:
        products = Product.query.filter_by(is_active=True).paginate(page=page, per_page=10)
...
def delete_product(id):
    ...
    add_log(f'Deactivated product {product.name}')
    db.session.commit()
...
def employees():
    page = request.args.get('page', 1, type=int)
    employees = Employee.query.filter_by(is_active=True).paginate(page=page, per_page=10)
...
def delete_employee(id):
    ...
    add_log(f'Deactivated employee {employee.name}')
    db.session.commit()
